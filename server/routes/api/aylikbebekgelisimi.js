...
router.get('/', async(req, res) => {
    ...
    try {
        res.send(await client.db('Kiduga').collection('AylikBebekGelisimi').find({}).toArray())
...
router.get('/:id', async(req, res) => {
    ...
    try {
        res.send(await client.db('Kiduga').collection('AylikBebekGelisimi').find({ _id: new mongodb.ObjectID(req.params.id) }).toArray())
    } catch { res.status(400).send(); } finally { client.close() }
});
...
router.put('/', async(req, res) => {
    ...
    try {
        await client.db('Kiduga').collection('AylikBebekGelisimi').updateMany({ _id: new mongodb.ObjectID(req.body.id) }, {
            $set: {
                detail: {
                    tr: {
                        name: req.body.name,
                        ...
                        sleepDesc: req.body.sleepDesc,
                        toiletDesc: req.body.toiletDesc,
                        activityDesc: req.body.activityDesc,
                    },
                    en: {
                        name: req.body.nameEn,
                        desc: req.body.descEn,
                        foodDesc: req.body.foodDescEn,
                        sleepDesc: req.body.sleepDescEn,
                        toiletDesc: req.body.toiletDescEn,
                        activityDesc: req.body.activityDescEn,
                    }
                },
...
router.post('/', async(req, res) => {
    ...
    try {
        ...
        deger = await client.db('Kiduga').collection('AylikBebekGelisimi').find().sort({ row_number: -1 }).limit(1).toArray()
        deger = deger[0].row_number
        deger += 1
    } catch { deger = 1 }

    await client.db('Kiduga').collection('AylikBebekGelisimi').insertOne({
...
router.delete('/', async(req, res) => {
    ...
    try {
        try {
            oyuncak = await client.db('Kiduga').collection('AylikBebekGelisimi').find({ _id: new mongodb.ObjectID(req.query.id) }).toArray()
        ...
        await client.db('Kiduga').collection('AylikBebekGelisimi').deleteOne({ _id: new mongodb.ObjectID(req.query.id) })
        res.status(200).send();
    } catch {
        res.status(400).send();
    } finally {
...
router.delete('/2', async(req, res) => {
    ...
    try {
        ...
        await client.db('Kiduga').collection('AylikBebekGelisimi').updateMany({ _id: new mongodb.ObjectID(req.query.id) }, {
            $unset: {
