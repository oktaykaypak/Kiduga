<template>
  <div>
    <div>
      <md-tabs >
        <md-tab id="deneme" md-label="Türkçe" to="oyuncakislemleri/oyuncakekle">
          <md-field>
            <label>Başlık</label>
...
export default {
  ...
  methods: {
    async Add() {
      try {
        await oyuncaklarservice
          .addData(this.name, this.link, this.desc, this.nameEn, this.descEn)
          .then((res) => {
            if (res.status === 200) {
              this.id = res.data.id;
              window.location = `http://localhost:8080/#/oyuncakislemleri/oyuncakduzenle?id=${this.id}`;
            }
          });
      } catch (err) {
        console.log(err);
