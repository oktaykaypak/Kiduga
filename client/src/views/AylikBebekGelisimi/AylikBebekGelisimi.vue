  ...
  <div>
    ...
    <div v-else>
      <div class="row" style="margin-bottom: 10px">
        <h1 style="width: 85%; margin-left: 2%">
          Aylık Bebek Gelişimi Listesi
        </h1>
        <button
          style="font-size: 18px; width: 10%; height: 70%"
          class="btn btn-success"
          @click="$router.push('/abebekgelisimi/aylikbebekgelisimiekle')"
        >
          Ekle
        </button>
      </div>
        ...
        >
          ...
          >
            <md-table-cell
              ...
              md-sort-by="name"
              style="width: 2%"
              >{{ abg.row_number != undefined ? abg.row_number : "-" }}</md-table-cell
            >
            <md-table-cell
              class="table-cell"
              md-label="Name"
              md-sort-by="name"
              >{{ abg.detail.tr.name }}</md-table-cell
            >
            <md-table-cell
              class="table-cell"
              md-label="Gender"
            ...
            >
              <router-link
                ...
                :to="`/abebekgelisimi/aylikbebekgelisimiduzenle?id=${abg._id}`"
                style="width: 45%"
                >Düzenle</router-link
              >
              <span
                ><button
                  v-on:click="Delete(abg._id)"
...
export default {
  ...
  computed: {
    orderedList: function () {
      return _.orderBy(this.abglist, "row_number");
    },
  },
  mounted() {},
  methods: {
    async GetABG() {
      this.abglist = await aylikbebekgelisimiservice.getData();
      this.isLoading = false;
    },
    async Delete(id) {
      await aylikbebekgelisimiservice.deleteData(id);
