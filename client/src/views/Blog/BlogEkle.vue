<template>
  <div>
    <md-tabs>
      <md-tab id="deneme" md-label="Türkçe" to="/blog/blogekle">
        <div class="row">
          <div class="col-sm-12">
            <md-field>
              <label>Başlık</label>
  ...
  methods: {
    ...
    async Add() {
      if (this.categoryId != null) {
        try {
          await blogservice
            .addBlog(this.name, this.desc, this.categoryId)
            .then((res) => {
              if (res.status === 200) {
                this.id = res.data.id;
                window.location = `http://localhost:8080/#/blog/blogduzenle?id=${this.id}`;
              }
            });
        } catch (err) {
          console.log(err);
