...
export default {
  ...
  methods: {
    ...
    async Add() {
      if (this.file != null || this.file2 != null) {
        try {
          ...
          await masallarservice.addMP3(this.id, this.mp3_url, this.mp3_url2);
    ...
    async Delete(language) {
      await masallarservice.deleteMP3(this.id, language);
      window.location.reload(1);
    },
    ...
    async GetOyuncaklar(id) {
      this.masallar = await masallarservice.getData2(id);
