<template>
  <div>
    <md-tabs>
      <md-tab id="deneme" md-label="EKLE" to="/muzikkategori/muzikkategoriekle">
        <md-field>
...
export default {
  ...
  methods: {
    async Add() {
      try {
        await muzikservice.addCategory(this.name).then((res) => {
          if (res.status === 200) {
            this.id = res.data.id;
            window.location = `http://localhost:8080/#/muzikkategori/muzikkategoriduzenle?id=${this.id}`;
          }
        });
      } catch (err) {
        console.log(err);
