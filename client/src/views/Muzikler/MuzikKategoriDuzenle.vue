<template>
  <div>
    ...
    <div v-else>
      <md-tabs>
        <md-tab id="deneme" md-label="DÜZENLE" to="/muzikkategori/muzikkategoriduzenle">
          <md-field>
...
export default {
  ...
  methods: {
    async GetCategories(id) {
      this.categories = await muzikservice.getCategoriesWId(id);
    ...
    async Edit() {
      try {
        await muzikservice
          ...
          .then((res) => {
            if (res.status === 200) {
              window.location = `http://localhost:8080/#/muzikkategori/muzikkategoriislemleri`;
            }
          });
      } catch (err) {
        console.log(err);
