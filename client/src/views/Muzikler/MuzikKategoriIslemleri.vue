  ...
  <div>
    ...
    <div v-else>
      <div class="row" style="margin-bottom: 10px">
          ...
          <h1 style="width: 85%;margin-left:2%">Müzik Kategorileri</h1>
          <button
            style="font-size: 18px; width: 10%;height:70%"
            class="btn btn-success"
            @click="$router.push('/muzikkategori/muzikkategoriekle')"
          >
            Ekle
          </button>
          ...
          >
            ...
            >
              <router-link
                ...
                :to="`/muzikkategori/muzikkategoriduzenle?id=${masal._id}`"
                style="width: 45%"
                >Düzenle</router-link
              >
              <span
...
export default {
  ...
  methods: {
    async GetMasallar() {
      this.masallar = await muzikservice.getCategories();
      this.isLoading = false;
    },
    async Delete(id) {
      await muzikservice.deleteCategories(id);
