...
export default {
  ...
  methods: {
    ...
    async Add() {
      if (this.file != null || this.file2 != null) {
        try {
          ...
          await muzikservice.addPhoto(this.id, this.img_url, this.img_url2);
    ...
    async Delete(language) {
      await muzikservice.deletePhoto(this.id, language);
      window.location.reload(1);
    },
    ...
    async GetOyuncaklar(id) {
      this.oyuncaklar = await muzikservice.getData2(id);
